...
import { IconArrowDown, IconArrowUp, IconArrowsLeftRight, IconBan, IconCheck, IconChevronDown, IconChevronUp, IconMinus, IconPlus } from './Icon'
...
function StrategyCard({
  ...
  const DirIcon  = strat.direction === 'BUY'
    ? IconArrowUp
          ...
          <div className="flex items-center gap-2">
            <span className={`text-xs font-bold px-2 py-0.5 rounded border ${
              ...
              <span className="inline-flex items-center gap-1.5">
                <DirIcon size={14} />
                <span>{strat.active ? strat.direction : 'INACTIF'}</span>
              </span>
            </span>
  ...
  if (!strats) {
    return (
      <div className="surface p-5">
        <h3 className="text-zinc-200 font-semibold text-sm mb-2">Stratégies de trading</h3>
    ...
    <div className="surface p-5">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="text-right">
          ...
          <p className="text-zinc-500 text-xs">{consensus.label}</p>
        </div>
      </div>
      ...
      <div className={`rounded-xl px-4 py-3 mb-4 border ${
        ...
        <div className="flex gap-4 mt-2 text-xs">
          ...
          {consensus.direction !== 'NEUTRAL' && (
            <span className={consensus.direction === 'BUY' ? 'text-green-400 font-bold inline-flex items-center gap-1.5' : 'text-red-400 font-bold inline-flex items-center gap-1.5'}>
              {consensus.direction === 'BUY' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />}
